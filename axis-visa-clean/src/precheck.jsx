...
import { Mark, Btn, Icon, Field, TextInput, SelectInput, ChoiceRow } from './primitives'
...
export function PreCheckModal({ open, onClose }) {
  const [step, setStep] = useState(0)
  const [form, setForm] = useState(emptyForm)
  const [touched, setTouched] = useState(false)
  const [done, setDone] = useState(false)
  const caseNo = useRef('AX-' + Math.floor(4000 + Math.random() * 5000))
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 80, background: 'rgba(17,22,27,.55)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: 'min(7vh,72px) 20px', overflowY: 'auto' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 600, maxWidth: '100%', background: 'var(--card)', borderRadius: 'var(--r-lg)', boxShadow: 'var(--shadow-lg)', overflow: 'hidden' }}>
        {/* header */}
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gap: 16 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', background: 'var(--paper-2)', border: '1px solid var(--line)', borderRadius: 'var(--r-sm)', padding: '11px 13px', marginTop: -4 }}>
                      <Icon name="info" size={16} color="var(--fg-3)" style={{ marginTop: 1 }} />
      ...
      <style>{`@media (max-width: 540px) { .step-label { display: none; } .field-pair { grid-template-columns: 1fr !important; } }`}</style>
    </div>
  )
}
