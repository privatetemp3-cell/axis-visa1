...
import { Mark, Eyebrow, Btn, Icon, WRAP, Field, TextInput, SelectInput, ChoiceRow } from './primitives'
...
export function VisaChecker() {
  const [form, setForm] = useState(vcEmpty)
  const [touched, setTouched] = useState(false)
  const [done, setDone] = useState(false)
  const refNo = useRef('AX-VR-' + Math.floor(40000 + Math.random() * 50000))
  ...
  const submit = () => {
    setTouched(true)
    if (!valid) return
    refNo.current = 'AX-VR-' + Math.floor(40000 + Math.random() * 50000)
    setDone(true)
  }
    ...
    <section id="visa-checker" style={{ background: 'var(--paper-2)', padding: '96px 0 84px', borderBottom: '1px solid var(--line)' }}>
      <div style={{ ...WRAP, display: 'grid', gridTemplateColumns: '.92fr 1.08fr', gap: 60, alignItems: 'start' }} className="visa-grid">
        ...
        <div>
          ...
          <Reveal delay={90}><h2 className="h2" style={{ margin: '0 0 14px' }}>Do you need a visa?</h2></Reveal>
          <Reveal delay={160}><p className="lead" style={{ margin: '0 0 22px', maxWidth: 460 }}>Free Visa Report sent to you via email.</p></Reveal>
          <Reveal delay={220}>
        ...
        <Reveal delay={160} y={26}>
          <div style={{ background: 'var(--card)', border: '1px solid var(--line-strong)', borderRadius: 'var(--r-lg)', boxShadow: 'var(--shadow-md)', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '28px 24px 24px' }}>
                ...
                <div style={{ border: '1px solid var(--line)', borderRadius: 'var(--r-md)', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '4px 16px 14px', background: 'var(--card)' }}>
                    ...
                    <VCReportRow label="Reference" value={refNo.current} />
                ...
                <p style={{ margin: '16px 2px 0', fontSize: 12.5, lineHeight: 1.55, color: 'var(--fg-3)' }}>
                  Your full report will confirm whether a visa is required, visa-free, or a transit visa may apply, with an overall confidence level and recommended next steps.
                </p>
      ...
      <style>{`
        @media (max-width: 900px) { #visa-checker .visa-grid { grid-template-columns: 1fr !important; gap: 40px !important; } }
        @media (max-width: 560px) { #visa-checker .visa-pair { grid-template-columns: 1fr !important; } }
      `}</style>
    </section>
  )
}
