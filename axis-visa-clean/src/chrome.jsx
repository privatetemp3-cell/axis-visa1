...
import { Logo, Btn, Icon, WRAP } from './primitives'
...
function NavLink({ children, target }) {
  ...
  return (
    <a href={'#' + target} onClick={(e) => { e.preventDefault(); scrollToId(target) }}
      ...
      style={{ fontFamily: 'var(--font-text)', fontSize: 15, fontWeight: 500, color: 'var(--ink)', position: 'relative', paddingBottom: 4, whiteSpace: 'nowrap', cursor: 'pointer', textDecoration: 'none' }}>
      {children}
...
export function SiteNav({ scrolled, onStart }) {
  return (
    <header style={{
      ...
      <div style={{ ...WRAP, height: scrolled ? 64 : 72, display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'height var(--dur) var(--ease)' }}>
        <a href="#top" onClick={(e) => { e.preventDefault(); scrollToId('top') }} style={{ cursor: 'pointer', textDecoration: 'none' }}><Logo size={20} /></a>
            ...
            {cols.map(([h, items]) => (
              <div key={h}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {items.map(([label, t], i) => (
                    <a key={i} href={'#' + t} onClick={(e) => { e.preventDefault(); scrollToId(t) }}
                      style={{ fontSize: 14, color: 'var(--fg-on-ink-2)', cursor: 'pointer', width: 'fit-content', whiteSpace: 'nowrap', textDecoration: 'none' }}
                      onMouseEnter={(e) => e.currentTarget.style.color = 'var(--paper)'}
                      onMouseLeave={(e) => e.currentTarget.style.color = 'var(--fg-on-ink-2)'}
                    >{label}</a>
                  ))}
                </div>
              </div>
