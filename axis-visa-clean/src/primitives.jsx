...
import {
  ArrowRight, ArrowDown, ArrowLeft, BookMarked, RotateCcw, Route, Compass,
  ShieldAlert, UserRound, Check, Minus, ListChecks, FileSearch, SearchX,
  FileCheck2, Banknote, Home, FileX, MapPinOff, PackageX, SearchCheck,
  Target, Layers, Receipt, UserRoundCheck, Globe, User, BadgeCheck, Mail,
  Scale, ChevronDown, X, Info, MailCheck,
} from 'lucide-react'

const ICONS = {
  'arrow-right': ArrowRight, 'arrow-down': ArrowDown, 'arrow-left': ArrowLeft,
  'book-marked': BookMarked, 'rotate-ccw': RotateCcw, 'route': Route,
  'compass': Compass, 'shield-alert': ShieldAlert, 'user-round': UserRound,
  'check': Check, 'minus': Minus, 'list-checks': ListChecks,
  'file-search': FileSearch, 'search-x': SearchX, 'file-check-2': FileCheck2,
  'banknote': Banknote, 'home': Home, 'file-x': FileX,
  'map-pin-off': MapPinOff, 'package-x': PackageX, 'search-check': SearchCheck,
  'target': Target, 'layers': Layers, 'receipt': Receipt,
  'user-round-check': UserRoundCheck, 'globe': Globe, 'user': User,
  'badge-check': BadgeCheck, 'mail': Mail, 'scale': Scale,
  'chevron-down': ChevronDown, 'x': X, 'info': Info, 'mail-check': MailCheck,
}

export function Icon({ name, size = 18, color = 'currentColor', style }) {
  const LucideIcon = ICONS[name]
  if (!LucideIcon) return null
  return <LucideIcon size={size} color={color} style={{ display: 'inline-flex', flexShrink: 0, ...style }} />
}

export function Mark({ size = 34, paper = false }) {
...
export function Logo({ paper = false, size = 21, tagline = false }) {
  return (
...
export function Eyebrow({ children, style, onInk = false }) {
  return (
    <div style={{
...
export function Btn({ children, variant = 'primary', onClick, paper = false, size = 'md', style, type = 'button' }) {
...
export const WRAP = { maxWidth: 1200, margin: '0 auto', padding: '0 40px', width: '100%', boxSizing: 'border-box' }
...
export function TextInput({ value, onChange, placeholder, type = 'text', invalid }) {
...
export function SelectInput({ value, onChange, options, placeholder, invalid }) {
  const [f, setF] = useState(false)
  return (
    <div style={{ position: 'relative' }}>
      <select
        ...
        style={{ ...fieldStyle(invalid), appearance: 'none', cursor: 'pointer',
          ...
          borderColor: invalid ? 'var(--danger)' : f ? 'var(--stamp)' : 'var(--line-strong)',
          boxShadow: f && !invalid ? '0 0 0 3px var(--stamp-tint)' : 'none', paddingRight: 36 }}>
        {placeholder && <option value="" disabled>{placeholder}</option>}
        {options.map((o) => <option key={o} value={o}>{o}</option>)}
      </select>
      <Icon name="chevron-down" size={16} color="var(--fg-3)" style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} />
    </div>
  )
}

export function ChoiceRow({ value, onChange, options }) {
  return (
